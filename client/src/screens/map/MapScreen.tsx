    ...
    <View style={S.ct}>
      ...
      <Modal visible={dialogVisible} transparent animationType="fade"><T style={D.ov} activeOpacity={1} onPress={closeDialog}><T style={D.cd} activeOpacity={1} onPress={() => {}}>
        {dialogData?.type === "normalChest" && (() => { const c = dialogData.data; const dist = userLocation ? getDist(userLocation.lat, userLocation.lng, c.coordinates.lat, c.coordinates.lng) : null; const inR = dist != null && dist <= 20;
          return <><Image source={{ uri: ICON_WHALEFALL }} style={D.eimg} /><Text style={D.tl}>鲸落</Text><Text style={{fontSize:16,fontWeight:"800",color:colors.primary,marginBottom:8}}>{c.label}</Text><View style={D.tr}><View style={[D.tg, { backgroundColor: colors.success + "18" }]}><Text style={[D.tt, { color: colors.success }]}>单人</Text></View><View style={[D.tg, { backgroundColor: inR ? colors.success + "18" : colors.info + "18" }]}><Text style={[D.tt, { color: inR ? colors.success : colors.info }]}>{inR ? "可拾取" : "20米内"}</Text></View></View>{dist != null && <Text style={D.dt}>距你 {dist}m</Text>}<Text style={D.dc}>靠近 20 米即可拾取这枚鲸落</Text>{cooldowns.normal > 0 ? <View style={{ backgroundColor: colors.warning + "15", borderRadius: 12, padding: 12, marginBottom: 12, width: "100%", alignItems: "center" }}><Text style={{ color: colors.warning, fontWeight: "700", fontSize: 14 }}>冷却中 · {Math.floor(cooldowns.normal / 60)}分{cooldowns.normal % 60}秒后可拾取</Text></View> : inR ? <T style={D.ub} onPress={() => handleUnlock(c._id)}><Text style={D.ut}>拾取鲸落</Text></T> : <T style={D.pb} onPress={closeDialog}><Text style={D.pt}>知道了</Text></T>}</>;
        })()}
